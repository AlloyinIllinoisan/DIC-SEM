...
tic = time.time()
ScriptName = 'Macros for Tws Fusion in ImageJ'
...
v = int(filestoredo[0][-32:-31]) #Number of regions
...
for j in range(v,v+1):
    for i in range(len(z)):
        L1 = '''open("{}/AlignedImages_step{}/Tws/step{}_{}.Tws_crop.tif");
selectWindow("step{}_{}.Tws_crop.tif");'''.format(directory,j,j,z[i],j,z[i])
        L2.append(L1)
    ...
    # L26 = 'run("Crop");'
    L27 ='run("Image Sequence... ", "select=['+'{}'.format(directory)+'/AlignedImages_step{}'.format(j)+'/Tws/Cropped/] dir=['+'{}'.format(directory)+'/AlignedImages_step{}'.format(j)+'/Tws/Cropped/] format=TIFF use");'
    L28 = 'run("Grid/Collection stitching", "type=[Positions from file] order=[Defined by TileConfiguration] directory=['+'{}'.format(directory)+'/AlignedImages_step{}'.format(j)+'/Tws/Cropped/] layout_file=Postprocessed_Tws_'+'step{}'.format(j)+'.txt fusion_method=[Linear Blending] regression_threshold=0.30 max/avg_displacement_threshold=2.50 absolute_displacement_threshold=3.50 computation_parameters=[Save memory (but be slower)] image_output=[Fuse and display]");'
    L29 = 'saveAs("Tiff", "'+'{}'.format(directory)+'/AlignedImages_step{}'.format(j)+'/Tws/Cropped/Fused_Tws_'+'step{}'.format(j)+'.tif");'
    L30 = 'close();'
    ...
    Confused.to_csv(path+'/MakeFuseTws_Redo_'+'step{}'.format(j)+'.ijm', float_format=None, index=False, mode ='w', header=False, sep='\t', quoting=csv.QUOTE_NONE, escapechar = '\t')
