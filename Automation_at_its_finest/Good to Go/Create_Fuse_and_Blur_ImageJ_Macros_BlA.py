...
for j in range(v,v+1):
    for i in range(len(z)):
        L1 = '''open("{}/AlignedImages_step{}/BlA/step{}_{}.BlA_crop.tif");
selectWindow("step{}_{}.BlA_crop.tif");'''.format(directory,j,j,z[i],j,z[i])
        L2.append(L1)
    ...
    L34 = 'saveAs("Tiff", "'+'{}'.format(directory)+'/AlignedImages_step{}'.format(j)+'/BlA/Cropped/Fused_BlA_'+'step{}'.format(j)+'.tif");'
    L35 = 'close();'
    ...
    Mars = np.array(L2, dtype=object)
    Jupiter = pd.Series(Mars)
    Saturn = pd.DataFrame(Jupiter).replace('  ',' ')
    Saturn.to_csv(path+'/MakeFuseBlA_'+'step{}'.format(j)+'.ijm', float_format=None, index=False, mode ='w', header=False, sep='\t', quoting=csv.QUOTE_NONE, escapechar = '\t')
