...
selectWindow("step{}_{}.GrX_crop.tif");'''.format(directory,j,j,z[i],j,z[i])
    ...
    Earth.to_csv(path+'/MakeFuseGrX_Redo_'+'step{}'.format(j)+'.ijm', float_format=None, index=False, mode ='w', header=False, sep='\t', quoting=csv.QUOTE_NONE, escapechar = '\t')
...
#    L3.append(L26)
    ...
    Belen.to_csv(path+'/MakeFuseGrXNoBlur_Redo'+'step{}'.format(j)+'.ijm', float_format=None, index=False, mode ='w', header=False, sep='\t', quoting=csv.QUOTE_NONE, escapechar = '\t')
