...
tic = time.time()
ScriptName = 'MARSHAWN HAS BROUGHT IT IN FOR A TOUCHDOWN!!!'
...
SecondEmail = lines[15][13:-1]

yag = yagmail.SMTP('{}'.format(WorkStationEmail), '{}'.format(Password))

contents = ['{} Completed'.format(ScriptName)]

yag.send(['{}'.format(SendToEmail), '{}'.format(SecondEmail)], '{}'.format(ProjectName), contents)
