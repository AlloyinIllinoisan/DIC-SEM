...
tic = time.time()
ScriptName = 'Make Cropped Folders and Postprocessed Text Files Batch File'
...
PythonScriptsDirectory = lines[12][26:-1] #Directory of the Aligned Images
...
cd "{}"
python Generate_Tiffs_from_Raw_Data_amended.py'''.format(PythonScriptsDirectory)
lemon.append(L2)
...
California.to_csv(BatchFileMacrosDirectory+'/Generate_Tiffs_from_Raw_Data_amended_Batch.bat', index=False, header=False, sep='\t', quoting=csv.QUOTE_NONE, escapechar = '\t')
